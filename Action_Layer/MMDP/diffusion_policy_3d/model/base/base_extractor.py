...
class BaseiDP3Encoder(nn.Module):
    def __init__(self, 
                 observation_space: Dict, 
        ...
        cprint(f"[BaseiDP3Encoder] point cloud shape: {self.point_cloud_shape}", "yellow")
        cprint(f"[BaseiDP3Encoder] depth point cloud shape: {self.depth_shape}", "yellow")
        cprint(f"[BaseiDP3Encoder] state shape: {self.state_shape}", "yellow")
        cprint(f"[BaseiDP3Encoder] iamge shape: {self.image_main_shape}", "yellow")
        ...
        self.img_encoder_outshape = self.image_encoder.output_shape()[1]
        ...
        cprint(f"[BaseDP3Encoder] output dim: {self.n_output_channels}", "red")
        ...
        if self.use_instr:
            ...
            cprint(f"[BaseDP3Encoder] instr output dim: {self.n_output_instr_channels}", "red")
    ...
    def forward(self, observations: Dict) -> torch.Tensor:
        ...
        final_feat = torch.cat([depth_feat, pn_feat, image_feat, state_feat], dim=-1)
