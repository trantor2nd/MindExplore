...
class MarsmindArmDataset(BaseDataset):
    def __init__(self,
            data_path, 
            pad_before=0,
            pad_after=0,
        ...
        super().__init__()
        cprint(f'Loading MarsmindArmDataset from {data_path}/{task_name}', 'green')
    ...
    def parse_metaaction(self, file_path):
        ...
        start_idx = 0
        for i in reversed(range(sample_step, num_steps, sample_step)):
            ...
            if np.abs(base_delta_ang) >= EPS or np.abs(base_vel_y) >= EPS:
                start_idx = i + sample_step
                break
        
        first_log_data = self.parse_log_file(os.path.join(file_path, "logs", log_files[start_idx]))
        qpos_first = first_log_data['joint_states_single_msg']['position']
        qvel_first = first_log_data['joint_states_single_msg']['velocity']

        for i in range(start_idx + sample_step, num_steps, sample_step):
            log_data = self.parse_log_file(os.path.join(file_path, "logs", log_files[i]))
            ...
            if first_idx == -1:
                ...
                if np.any(qpos_delta > EPS) or qpos_delta[-1] > EPS/2:
                    first_idx = i

                    first_log_data = self.parse_log_file(os.path.join(file_path, "logs", log_files[i - sample_step]))
                    ...
                    first_depth_img = cv2.imread(os.path.join(file_path, "p0", "depth", log_files[i - sample_step].replace('.log', '.png')), \
                                                cv2.IMREAD_UNCHANGED)
                    first_depth_data = depth_to_lidar(first_depth_img)
                    distances = np.sqrt(first_depth_data[:, 0]**2 + first_depth_data[:, 2]**2)
                    first_depth_data = first_depth_data[distances <= RADIUS_DEPTH]
                    first_depth_data = point_process.uniform_sampling_numpy(first_depth_data[None, :], self.num_points)[0]
                    depth.append(first_depth_data)
                    
                    if USE_PC and not USE_DEPTH_1:
                        first_pc_data = pd.read_csv(os.path.join(file_path, "pc", log_files[i - sample_step].replace('.log', '.csv')))
                        first_pc_data = first_pc_data[['x', 'y', 'z']].to_numpy()
                        distances = np.sqrt(first_pc_data[:, 0]**2 + first_pc_data[:, 1]**2)
                        first_pc_data = first_pc_data[distances <= RADIUS_PC]
                        # 删除 x 在 (-0.5, 0) 且 y 在 (-0.5, 0.5) 区域内的点
                        mask = ~((first_pc_data[:, 0] > -0.5) & (first_pc_data[:, 0] < 0) & (first_pc_data[:, 1] > -0.5) & (first_pc_data[:, 1] < 0.5))
                        first_pc_data = first_pc_data[mask]
                    elif USE_DEPTH_1 and not USE_PC:
                        first_depth_img_1 = cv2.imread(os.path.join(file_path, "p1", "depth", log_files[i - sample_step].replace('.log', '.png')), \
                                                cv2.IMREAD_UNCHANGED)
                        first_pc_data = depth_to_lidar(first_depth_img_1)
                        distances = np.sqrt(first_pc_data[:, 0]**2 + first_pc_data[:, 2]**2)
                        first_pc_data = first_pc_data[distances <= RADIUS_DEPTH_1]
                    else:
                        raise ValueError('no implementation.')
                    ...
                    img_main_path.append(os.path.join(file_path, "p0", "rgb", log_files[i - sample_step].replace('.log', '.png')))
                    img_wrist_path.append(os.path.join(file_path, "p1", "rgb", log_files[i - sample_step].replace('.log', '.png')))
        ...
        instr = re.sub(r'(?i)\b(Being careful|be careful|taking care)\b.*', '', instr)
