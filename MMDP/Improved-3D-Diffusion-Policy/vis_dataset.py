...
for episode_idx, episode_end in enumerate(all_episode_ends):
    ...
    save_dir = f"/media/Image_Lab/hsb/vis_data/{dataset_path.split('/')[-1]}/{episode_idx}"
    if vis_cloud:
    ...
    if vis_cloud:
        # to video
        os.system(f"ffmpeg -r 10 -i {save_dir}/%d.png -vcodec mpeg4 -y /media/Image_Lab/hsb/vis_data/{dataset_path.split('/')[-1]}/{episode_idx}.mp4")
